    ...
    {% for source in graph.sources.values() %}
      {%- if source.source_name == model_grp -%}
        {%- if source.meta.tgt_tbl == model_id -%}
          ...
          {# Schema comparison from Source to Target, The load is proceeded, only when the comparison is a matched case #}
          ...
          {% if src_columns_list | sort == tgt_columns_list | sort %}
            ...
            {% if prevalidation_check == 'y' %}
              {# This macro does the pre-validation checks and captures all the bad records , specifically related to the composite key(Null chk & Duplicate chk, or any other check defined) from the source , and writes in to a pre validation table dbt_monitor.dbt_model_pre_validation_check_t #}
              ...
              {% set create_sql %}
                ...
                from {{ src_tbl }} src
                join (select {{ pk_columns_str }}, max({{ inc_load_ts_column }}) as {{ inc_load_ts_column }} from {{ src_tbl }} group by all) tgt
