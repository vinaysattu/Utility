    ...
    {% for source in graph.sources.values() %}
      {% if source.source_name == model_grp %}
        {%- if source.meta.tgt_tbl == model_id -%}
          ...
          {# Columns exclusion for the schema comparison Source to Target #}
          ...
          {% if src_columns_list | sort == tgt_columns_list | sort %}
            ...
            {% elif load_type == 'incremental' %}
              {# When the Load Type is Incremental, the load process is Delete and Inserts the new incremented records from/to the target table #}
              {% set delete_sql %}
                begin;
                delete from {{ tgt_tbl }}
              ...
              {% set insert_sql %}
                ...
                select
                  ...
                  current_timestamp()::timestamp_ntz(9) as etl_load_ts,
                  current_timestamp()::timestamp_ntz(9) as etl_updt_ts
                from {{ src_tbl }} src where src.{{ inc_load_ts_column }} > '{{ inc_load_ts }}';
                commit;
              {% endset %}
              {% do run_query(insert_sql) %}
            {% else %}
