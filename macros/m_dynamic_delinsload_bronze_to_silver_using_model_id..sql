        ...
        {%- if source.meta.tgt_tbl == model_id -%}
          ...
          {% if src_columns_list | sort == tgt_columns_list | sort %}
            ...
            {% set delete_sql %}
              ...
              where exists (
                ...
                where {{ pk_join_condition }}
                  and src.etl_load_ts > (select COALESCE(MAX(etl_updt_ts), '1980-01-01') from {{ tgt_tbl }})
              );
              commit;
            {% endset %}
            ...
            {% set insert_sql %}
              ...
              from {{ src_tbl }} src
              where src.etl_load_ts > (select COALESCE(MAX(etl_updt_ts), '1980-01-01') from {{ tgt_tbl }});
              commit;
            {% endset %}
