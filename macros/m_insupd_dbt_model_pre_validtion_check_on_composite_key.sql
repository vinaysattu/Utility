{% macro m_insupd_dbt_model_pre_validation_check_on_composite_key(model_id, model_grp, src_tbl, tgt_tbl, key_column_list, src_json_column, v_batch_id) %}
{# This macro does the pre-validation checks and captures all the bad records , specifically related to the composite key(Null chk & Duplicate chk, 
or any other check defined) from the source , and writes in to a pre validation table dbt_monitor.dbt_model_pre_validation_check_t #}
{% set merge_sql -%}
begin;
...
on (target.model_id = source.model_id and target.model_grp = source.model_grp and target.key_column = source.key_column 
and target.validation_type = source.validation_type 
and target.src_load_ts=source.src_load_ts)
when matched then
update set 
  ...
  target.success_flg = source.success_flg,
...
values (source.model_id, source.model_grp, source.key_column, source.validation_type, source.issue_record_count, source.json_value, source.success_flg, 
