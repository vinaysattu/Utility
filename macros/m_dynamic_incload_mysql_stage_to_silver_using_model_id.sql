...
{% for source in graph.sources.values() %}
  {% if source.source_name == v_model_grp %}
    {%- if source.meta.tgt_tbl == v_model_id -%}
      ...
      {% set v_src_columns_list = [] %}
      {% for col in src_columns_result['data'] %}
        {% set _ = v_src_columns_list.append(col[0]) %}
      {% endfor %}
      ...
      {% set v_tgt_columns_list = [] %}
      {% for col in tgt_columns_result['data'] %}
        {% set _ = v_tgt_columns_list.append(col[0]) %}
      {% endfor %}
      ...
      {%- for col in v_tgt_columns_list -%}
        {%- if col not in columns_to_remove -%}
          {%- set _ = filtered_tgt_columns_list.append(col) -%}
        {%- endif -%}
      {%- endfor -%}
      {%- set v_tgt_columns_list = filtered_tgt_columns_list -%}

      {%- set filtered_src_columns_list = [] -%}
      {%- for col in v_src_columns_list -%}
        {%- if col not in columns_to_remove -%}
          {%- set _ = filtered_src_columns_list.append(col) -%}
        {%- endif -%}
      {%- endfor -%}
      {%- set v_src_columns_list = filtered_src_columns_list -%}

      {{ ftl_utils.m_capture_schema_drift_for_model_id(v_pipeline_name, v_model_grp, v_model_id, v_batch_id) }}
      ...
      {% if replication_status == 'running' %}
        ...
        {% set insert_columns_str = v_tgt_columns_list | join(', ') %}
        {% set select_columns_str = v_tgt_columns_list | join(', src.') %}
        ...
        {% set merge_sql %}
          ...
          using (
            ...
            {% if v_load_type == 'full' %}
            join (
              select {{ pk_columns_str }}, max({{ v_inc_load_ts_column }}) as {{ v_inc_load_ts_column }}
              from {{ v_src_tbl }}
              group by {{ pk_columns_str }}
            ) tgt
            on {{ pk_join_condition }} and src.{{ v_inc_load_ts_column }} = tgt.{{ v_inc_load_ts_column }}
            {% endif %}
          ...
          when not matched then insert (
            {{ insert_columns_str }}, src_load_ts, batch_id, etl_load_ts, etl_updt_ts
          )
          values (
            src.{{ insert_columns_str }}, src.src_load_ts, src.batch_id, src.etl_load_ts, src.etl_updt_ts
          );
          commit;
        {% endset %}
