    ...
    {% if execute %}
        ...
        {% for source in graph.sources.values() %}
            {%- if source.source_name == v_model_grp and source.meta.tgt_tbl == v_model_id -%}
				...
				{% set replication_status = fl_utils.m_mysql_agent_check(v_model_grp, v_model_id_short) %}
                {% if replication_status != 'running' %}
					...
					{% set journal_sql %}
						...
						and upper(regexp_substr(table_name,upper('{{ v_src_table }}')||'_'||split_part(table_name, '_', -1))) =upper('{{ v_src_table }}')||'_'||split_part(table_name, '_', -1)
					...
					{% for row in journal_tbls %}
						...
						{% set union_sql = "
							select 
								primary_key__pk, event_type, most_significant_position, least_significant_position, seen_at, sf_metadata,
								object_delete(object_construct(*), 'PRIMARY_KEY__PK', 'EVENT_TYPE', 'MOST_SIGNIFICANT_POSITION', 'LEAST_SIGNIFICANT_POSITION', 'SEEN_AT', 'SF_METADATA') as attribute_value_json,
                                '" ~ v_src_db ~ ".journals.\"" ~ tbl_name ~ "\"' as src_journal_tbl
							from " ~ v_src_db ~ ".journals.\"" ~ tbl_name ~ "\"
							where primary_key__pk is not null and " ~ v_inc_load_ts_column ~ " > " ~ v_inc_load_ts ~ ""
						%}
					...
					{% set insert_sql %}
						...
						insert into {{ v_tgt_tbl }} 
							(primary_key__pk, event_type, most_significant_position, least_significant_position, seen_at, sf_metadata, attribute_value_json, 
