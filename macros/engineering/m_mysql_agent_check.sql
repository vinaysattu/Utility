...
{% for attempt in range(retries) %}
    {% set replication_status_query %}
       ...
       {# and started_at >= DATE_TRUNC('hour', CURRENT_TIMESTAMP) 
        and started_at < DATE_TRUNC('hour', DATEADD(hour, 1, CURRENT_TIMESTAMP))
        ORDER BY 
            started_at DESC #}
        LIMIT 1
    {% endset %}
    ...
    {% if status == 'running' %}
        {{ log("Replication is currently running. Attempt " ~ (attempt + 1) ~ " of " ~ retries, info=True) }}
        {% if attempt < retries - 1 %}
            {{ log("Sleeping for " ~ sleep_seconds ~ " seconds before retrying...", info=True) }}
            {% do run_query("CALL SYSTEM$WAIT(" ~ sleep_seconds ~ ")") %}
        {% else %}
            {{ log("Max retries reached. Exiting...", info=True) }}
            {% do return(None) %}
        {% endif %}
    {% else %}
        {{ log("Replication status: " ~ status, info=True) }}
        {% do return(status) %}
    {% endif %}
{% endfor %}
{% endmacro %}