{% macro m_process_deletions_from_silver_using_del_flg(p_pipeline_name, model_grp, model_id) %}
	{# Deletion of Silver Table records (Hard Delete), This macro compares and fetches all the deleted records by doing target-source,if the records are deleted in stage by OGG, these delete records are then moved to tbl_del_t tables in the same silver layer, and then deleted fromt the target table. #}
	{# Prepares Target table using the full length target table , which is pulled from source yaml #}
	...
	{# Fetch Full length stage table name using the model id from yaml #}
	...
	{% if del_flg == 'y' %}
		...
		{% set table_base = tgt_tbl_short[:tgt_tbl_short|length - 2] %}
		{% set table_suffix = tgt_tbl_short[-2:] %}
		{% set del_table_name = table_base ~ '_del' ~ table_suffix %}
		...
		{# Step 0: Create the deletion table if it doesn't exist, using the same structure as the silver table #}
		{% set create_sql -%}
			begin;
