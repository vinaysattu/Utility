        ...
        {%- for source in graph.sources.values() -%}
            {%- if source.source_name == model_grp -%}
                {%- if source.meta.tgt_tbl == model_id -%}
                    ...
                    {%- set column_mapping = source.meta.column_mapping if 'column_mapping' in source.meta else {} -%}
                    ...
                    {% do log("Setting File Archive Path : " ~ archive_path, info=True) %}
					...
					{# In this step based on the file_header flag,  The type of copy activity is chosen and,  which first copies the data load from source file to the temporary table which is created and thn this is moved to main Bronze table with its respective mapping created in the column mapping. #}
                    {%- if file_header == 'y' -%}
						{% set copy_sql -%}
							begin;
