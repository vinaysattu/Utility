{% macro m_model_load_with_model_grp(p_model_grp, p_full_model_name='ALL', p_is_backfill='N') %}

{%- set v_model_grp = p_model_grp %}
{%- set v_model_name = p_full_model_name %}
...
{% do log("Model Name: " ~ v_model_name, info=True) %}
...
{% do log("Model_ID list: " ~ v_model_id_list, info=True) %}

{# Filter model_id_list if model_name is not ALL #}
{%- if v_model_name != 'ALL' %}
    {%- set filtered_model_ids = [] %}
    {%- for id_string in v_model_id_list -%}
        {%- if id_string == v_model_name|string %}
            {%- do filtered_model_ids.append(id_string) %}
        {%- endif %}
    {%- endfor -%}
    {%- set v_model_id_list = filtered_model_ids -%}
{%- endif %}

{% do log("Filtered model_list: " ~ v_model_id_list, info=True) %}

{%- if v_batch_status == 'running' %}

    {# Initiate the load attributes inside the table dbt_monitor.dbt_model_audit_t #}
    ...
    {% do log("All the Model meta data is Initiated, Proceeding with the model load process..", info=True) %}
    ...
    {%- for v_model_id in v_model_id_list %}
        ...
        {%- set t_batch_id = fl_utils.m_get_batch_id_from_dbt_model_audit( v_pipeline_name, v_model_grp, v_model_id_full, v_batch_id ) %}

        {# Fetch Model load status from the (dbt_monitor.dbt_model_audit_t) Table #}
        {%- set t_model_load_status = fl_utils.m_get_model_load_record_from_dbt_model_audit( v_pipeline_name, v_model_grp, v_model_id_full, v_batch_id ) %}
        ...
        {% else %}
            ...
            {{ fl_utils.m_upd_pre_model_load_attribs_vals_for_model_record_in_dbt_model_audit( v_pipeline_name, v_model_grp, v_model_id_full, v_batch_id ) }}
            ...
            {% if v_pipeline_name == 'mfcs' %}
                ...
                {% elif v_tgt_layer == 'silver' %}
                    ...
                    {# Deletion of Silver Table records (Hard Delete), The below macro compares and fetches all the deleted records by doing target-source, 
                    If the records are deleted in stage by OGG, these delete records are then moved to tbl_del_t tables in the same silver layer, and then deleted from the target table. #}
            ...
            {% elif v_pipeline_name == 'wfm' %}
                ...
                {% if v_tgt_layer == 'bronze' %}
                    {# Load from External Stage to Bronze for each model sequentially, considering the specific attributes for a model from yaml.#}
                    {{ fl_utils.m_dynamic_load_extstage_to_bronze_using_model_id( v_pipeline_name, v_model_grp, v_model_id_full, v_batch_id ) }}
                {% elif v_tgt_layer == 'silver' %}
                    {# Load from Bronze to Silver (Delete and then Insert) for each model sequentially, considering the specific attributes for a model from yaml.#}
            ...
            {% elif v_pipeline_name == 'pim' or v_pipeline_name == 'pim_v9' %}
                ...
                {% if v_tgt_layer == 'silver' %}
                    {# Load which is pre-validated(Divert bad records to dbt_monitor.dbt_model_pre_validation_check_t table for the checks in place) from Bronze to Silver
                    for each model sequentially with the incremental strategy specific to the model. #}
            ...
            {% elif v_pipeline_name == 'dom' or v_pipeline_name == 'wm' %}
                ...
                {% elif v_tgt_layer == 'silver' %}
                    ...
                    {% endif %}
                    {{ fl_utils.m_mysql_process_deletions_from_silver_using_model_id( v_pipeline_name, v_model_grp, v_model_id ) }}
                {% endif %}
