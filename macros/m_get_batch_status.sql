{% macro m_get_batch_status(p_pipeline_name, batchid) %}
  {# Fetch the batch status using the pipeline name from DBT_MONITOR.dbt_model_batch_t, where the batch_status = 'running',
     for it to process the load, if the batch_status = 'completed', the load will be skipped #}
